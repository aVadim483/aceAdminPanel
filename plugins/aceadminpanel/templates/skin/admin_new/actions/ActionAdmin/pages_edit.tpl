...
<script type="text/javascript" src="{cfg name='path.root.engine_lib'}/external/tinymce-jq/tiny_mce.js"></script>
    ...
    {else}
{include file='modal_load_img.tpl' sToLoad='page_text'}
<script type="text/javascript">
    jQuery(function($){
...
{hook run='plugin_page_form_add_begin'}
    ...
    <p>
        ...
        {foreach from=$aPages item=oPage}
            <option style="margin-left: {$oPage->getLevel()*20}px;" value="{$oPage->getId()}" {if $_aRequest.page_pid==$oPage->getId()}selected{/if}>
    ...
    <p>
        ...
        <input type="text" id="page_title" class="input-text input-width-full" name="page_title" value="{$_aRequest.page_title}" class="input-wide" />
    </p>


    <p>
        <label for="page_url">{$aLang.plugin.page.create_url}:</label>
        <input type="text" class="input-text input-width-full" id="page_url" name="page_url" value="{$_aRequest.page_url}" class="input-wide" />
    </p>
...
{hook run='plugin_page_form_add_end'}
    <p>
        ...
        <button class="btn" onclick="window.location='{router page='admin'}pages/'; return false;" />{$aLang.plugin.page.create_submit_cancel}</button>
    </p>
